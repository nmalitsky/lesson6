...
routerAPIv1.route('/users')
	...
	.get((req, res, next) => {
		...
		Object.keys(users).forEach(user_id => {
			...
			if(fields) user = utils.checkFields(user, fields);
			arrUsers.push(user);
		});

		// offset & limit
		let offset = parseInt(req.query.offset || 0); 
		let limit = parseInt(req.query.limit || arrUsers.length);
		if(isNaN(offset) || isNaN(limit)) {
		        let errorParams = new Error('error input parameters (limit, offset)');
      			errorParams.status = 400;
      			return next(errorParams);
		}
		res.status(200).end(utils.fmtJSON(arrUsers.slice(offset, offset + limit)));
	})
...
routerAPIv1.route('/users/:user_id')
	...
	.get((req, res) => {
		let user = users[req.params.user_id]
		res.status(200).end(utils.fmtJSON(user));
	})
